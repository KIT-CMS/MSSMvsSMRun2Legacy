...
# set other to 0
    ...
    os.system('combineTool.py -m "60,80,95,100,120,125,130,140,160,180,200,250" -M AsymptoticLimits --rAbsAcc 0 --rRelAcc 0.0005 --boundlist input/mssm_boundaries.json --freezeParameters r_ggH,r_qqX,r_ggX --setParameters r_ggH=0,r_bbH=0,r_qqX=0,r_ggX=0 --redefineSignalPOIs r_bbH -d %(directory)s/%(year_str)s/%(channel_str)s/ws.root --there -n ".bbH.v2_freezeggH" --task-name bbH_full_combined_%(analysis)s_%(channel)s_%(year)s_%(output)s_freezeggH --X-rtd MINIMIZER_analytic --cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance 0.01 -v 1 --job-mode \'SGE\' --prefix-file ic --sub-opts "-q hep.q -l h_rt=3:0:0" ' % vars())
