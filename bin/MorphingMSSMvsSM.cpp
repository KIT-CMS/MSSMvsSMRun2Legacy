...
int main(int argc, char **argv) {
  ...
  // Add systematics
  dout("Add systematics AddMSSMvsSMRun2Systematics :", ! no_emb);
  ch::AddMSSMvsSMRun2Systematicstemp(cb, true, ! no_emb, true, true, true, era, mva);
  ...
  if (auto_rebin) {
    ...
    auto rebin = ch::AutoRebin().SetBinThreshold(15.0).SetBinUncertFraction(0.9).SetRebinMode(1).SetPerformRebin(true).SetVerbosity(1);
    rebin.Rebin(cb, cb);
  }
