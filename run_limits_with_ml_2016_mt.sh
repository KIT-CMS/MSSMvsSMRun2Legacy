...
if [[ $MODE == "initial" ]]; then
    ...
    morph_parallel.py --output ${defaultdir}/datacards \
        --analysis ${analysis} \
        --eras 2016 \
        --category_list ${CMSSW_BASE}/src/CombineHarvester/MSSMvsSMRun2Legacy/input/sm_neuralnet_categories_debug.txt \
        --variable nnscore \
        --sm \
    ...
    morph_parallel.py --output ${defaultdir}/datacards \
        --analysis ${analysis} \
        --eras 2016 \
        --category_list ${CMSSW_BASE}/src/CombineHarvester/MSSMvsSMRun2Legacy/input/mssm_new_categories_debug.txt \
    ...
    -i ${datacarddir}/2016/mt/ 2>&1 | tee -a ${defaultdir}/logs/workspace.txt
    ############
    # job setup creation
    ############
    ...
    -d ${datacarddir}/2016/mt/ws_mh125.root \
    --job-mode 'condor' \
    --task-name $taskname \
    --dry-run \
    --redefineSignalPOI x \
