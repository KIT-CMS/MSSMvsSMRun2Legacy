...
parser.add_argument('--fit', '-f', help='background only fit result')
...
# get background only fit result
fit_file = args.fit
infile = ROOT.TFile(fit_file.split(':')[0])
fit = infile.Get(fit_file.split(':')[1])

data_bkgOnly = {}

fit_list = fit.floatParsFinal()

for iPar in range(0, fit_list.getSize()):
  var = fit_list.at(iPar)
  name = var.GetName()
  central = var.getVal()
  error = var.getError()
  #print name, central, error
  data_bkgOnly[name] = (central, error)
...
for page in xrange(n):
    ...
    g_pulls_bkgOnly = ROOT.TGraphAsymmErrors(n_params)
    ...
    for p in xrange(n_params):
        ...
        if pdata[p]['type'] != 'Unconstrained':
            ...
            #bkgOnly case (with symetric errors - can be updated)
            x=data_bkgOnly[name]
            pull = x[0] - pre[1]
            pull = (pull/pre_err_hi) if pull >= 0 else (pull/pre_err_lo)
            pull_hi = x[1]+x[0] - pre[1]
            pull_hi = (pull_hi/pre_err_hi) if pull_hi >= 0 else (pull_hi/pre_err_lo)
            pull_hi = pull_hi - pull
            pull_lo = x[0]-x[1] - pre[1]
            pull_lo = (pull_lo/pre_err_hi) if pull_lo >= 0 else (pull_lo/pre_err_lo)
            pull_lo =  pull - pull_lo
            g_pulls_bkgOnly.SetPoint(i, pull, float(i) + 0.5)
            g_pulls_bkgOnly.SetPointError(
                i, pull_lo, pull_hi, 0., 0.)
        else:
            # Hide this point
            g_pulls.SetPoint(i, 0., 9999.)
            g_pulls_bkgOnly.SetPoint(i, 0., 9999.)
            y1 = ROOT.gStyle.GetPadBottomMargin()
            y2 = 1. - ROOT.gStyle.GetPadTopMargin()
            ...
            if name not in ['r_bbH', 'r_ggH']:
              x=data_bkgOnly[name]
              s_nom, s_hi, s_lo = GetRounded(x[0], x[1], x[1]) 
              x1 = x1 + (1 - pads[0].GetRightMargin() -x1)*1.3/2.
              text_entries_bkgOnly.append((x1, y1, '%s^{#plus%s}_{#minus%s}' % (s_nom, s_hi, s_lo)))
            redo_boxes.append(i)
    ...
    for entry in text_entries_bkgOnly:
        latex2.DrawLatex(*entry)
    ...
    g_pulls.Draw('PSAME')
    plot.Set(g_pulls_bkgOnly, MarkerSize=0.8, LineWidth=2, MarkerColor=2, LineColor=2, LineStyle=2)
    g_pulls_bkgOnly.Draw('PSAME')
    ...
    legend = ROOT.TLegend(0.02, 0.02, 0.55, 0.06, '', 'NBNDC')
    legend.SetNColumns(4)
    legend.AddEntry(g_pulls, 'Pull', 'LP')
    legend.AddEntry(g_pulls_bkgOnly, 'Pull (B-only)', 'LP')
